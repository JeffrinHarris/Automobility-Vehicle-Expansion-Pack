    ...
    with zipfile.ZipFile(zipfile_path, 'w', zipfile.ZIP_DEFLATED) as packfile:
        for root, _, files in os.walk(resource_folder):
            for file in files:
                ...
                zipped_path = os.path.join(resource_folder,
                    os.path.relpath(file_path, resource_folder))
...
if __name__ == "__main__":
    ...
    try:
        with open("./workspace.txt", "x") as f:
            f.write("# Workspace-only variables (not committed to git)\n")
            f.write("# build.assets.out=C:/path/to/minecraft/resourcepacks\n")
            f.write("# build.data.out=C:/path/to/minecraft/datapacks\n")
            f.write("# build.mod.out=C:/path/to/minecraft/mods\n")
